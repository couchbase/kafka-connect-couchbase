...
Provide a `couchbase.document.id` property whose value is a JSON Pointer identifying the document ID field.
...
If the connector fails to locate the document ID field, it will fall back to using the Kafka key or `topic/partition/offset` as described above.
