...
* *Deprecated*.
Prior to version 3.4.6, enabling this feature can cause `OutOfMemoryError` during initial backfill, since snapshot contents are buffered in memory and the first snapshot contains the entire contents of the database.
In version 3.4.6 and later, enabling this feature has no effect.
This configuration property will be removed entirely in a future release.
* Type: boolean
* Importance: low
* Default: `false`
